...
applyingJCovSettingsToMakefileForTests()
{
  ...
  if [[ $VERSION == *8* ]]; then
	...
	sed -i "s/-vmoption:-Xmx512m.*/-vmoption:-Xmx512m -xml:verify -jcov\/classes:\$\(ABS_PLATFORM_BUILD_ROOT\)\/images\/jmods\/java.base.jmod  -jcov\/source:\$\(ABS_PLATFORM_BUILD_ROOT\)\/images\/jdk\/lib\/src.zip  -jcov\/include:*/" TestCommon.gmk
  fi
  pwd
  cd "$OPENJDK_DIR" || exit
}
