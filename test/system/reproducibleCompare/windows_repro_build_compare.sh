...
if [ "$#" -eq 1 ]; then
  SBOM_URL=$(realpath $1/*sbom*.json)
  TARBALL_URL=$(realpath $1/*jdk*.zip)
elif [ "$#" -eq 2 ]; then
  SBOM_URL="$1"
  TARBALL_URL="$2"
else 
  echo "Usage: $0 LOCAL_PATH_OF_SBOM_AND_JDK or $0 SBOM_URL/SBOM_PATH JDKZIP_URL/JDKZIP_PATH"
  echo ""
  echo "1. LOCAL_PATH_OF_SBOM_AND_JDK - path of both sbom and jdk"
  echo ""
  echo "2. SBOM_URL/SBOM_PATH - should be the FULL path OR a URL to a Temurin JDK SBOM JSON file in CycloneDX Format"
  ...
  echo ""
  echo "3. JDKZIP_URL/JDKZIP_PATH - should be the FULL path OR a URL to a Temurin Windows JDK Zip file"
